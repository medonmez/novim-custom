...
- `TASK-017` passed local review at `6b902a8`, but merged PR #31 exposed a
  `shellcheck` `SC1003` failure at `install.sh:171`; acceptance is withheld
  until both installer copies pass the same-task correction review. The review
  records a separately repaired installed-`novim` probe incident; hosted
